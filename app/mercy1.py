...
def display_sentinel_images():
    ...
    for img_file, meta in SENTINEL_IMAGES.items():
        ...
        if os.path.exists(img_path):
            try:
                ...
                if img.mode != "RGB":
                    img = img.convert("RGB")
                st.image(img, caption=f"{meta['label']}", use_container_width=True)
            except Exception as e:
